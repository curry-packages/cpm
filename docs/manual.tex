...
\begin{lstlisting}
> cypm list
\end{lstlisting}\pindex{list}
%
The command
%
\begin{lstlisting}
> cypm info PACKAGE
\end{lstlisting}\pindex{info}
%
...
standard:\footnote{\url{http://www.semver.org}}%
\index{version format}\index{semantic versioning}
each version number consists of 
...
\code{package.json}\pindex{package.json}
...
\code{src} directory inside your project's directory.
One can also use the command\pindex{init}
\begin{lstlisting}
cypm init
\end{lstlisting}
to initialize the current directory
with a minimal version of \code{package.json}
and a few other files (like \code{README} or license information).
When starting a new project, one can also use the command\pindex{new}
\begin{lstlisting}
cypm new <project-name>
\end{lstlisting}
to create a new project directory with a 
\code{package.json} file and some other useful template files
(look into the output of this command).
...
\end{lstlisting}
%
Now you can run\pindex{install}
\begin{lstlisting}
cypm install
\end{lstlisting}
to install all dependencies of the current package
and start your interactive Curry environment with the command\pindex{curry}
\begin{lstlisting}
cypm curry
\end{lstlisting}
You will be able to load the JSON package's modules in your Curry session.


\subsection{Installing and Updating Dependencies}

To install the current package's dependencies,
run \code{cypm install}.\pindex{install} This will
...
\code{cypm upgrade}\pindex{upgrade}
...
one can use the command\pindex{checkout}
\begin{lstlisting}
...
one can use the command\pindex{install}
\begin{lstlisting}
...
Any parameters given to \ccode{cypm curry} will be passed along verbatim
to the Curry system.
For example, the following command will start the Curry system,
prints the result of evaluating the expression \code{39+3}
and then quits.
\pindex{curry}
\begin{lstlisting}
...
\end{lstlisting}
%
The command \ccode{cypm repl} is just a synonym for \ccode{cypm curry}
so that one can replace the previous command by
\pindex{repl}
\begin{lstlisting}
> cypm repl :eval "39+3" :quit
\end{lstlisting}
%
To execute other commands with the package's dependencies available,
such as tools like CurryCheck \cite{Hanus16LOPSTR}
with its executable \code{curry-check},
you can use the \ccode{cypm exec} command.
This command will set the \code{CURRYPATH} environment variable
and then executes the command given after \ccode{exec}.
...
in your application, one can use the following commands:\pindex{add}
%
\begin{lstlisting}
...
\code{diff}\pindex{diff}
can be used to compare the types and behavior of a package's public
...
\verb|{-# TERMINATE -#}|\pindex{TERMINATE} or
\verb|{-# PRODUCTIVE -#}|\pindex{PRODUCTIVE} to annotate such
...
\verb|{-# NOCOMPARE -#}|\pindex{NOCOMPARE}
...
CPM offers two kinds of equivalence tests:\index{equivalence}
\begin{itemize}
\item
...
\subsection{Testing a Package}
\label{sec:testing-a-package}

Before publishing a package and make it available to other developers
(see Section~\ref{sec:publishing-a-package}), it should be
carefully checked and tested.
For this purpose, CPM offers two commands:
%
\begin{lstlisting}
> cypm check
\end{lstlisting}\pindex{check}
%
This commands checks the current package specification
for consistency of the dependencies.
All source code modules are compiled and their actual imports
are examined. If there is some imported module
which is not part of a package specified in the dependencies,
an error is reported.
If there is some package in the dependency specification
which is not used, a warning is reported.
%
\begin{lstlisting}
> cypm test
\end{lstlisting}\pindex{test}
%
This tests the current package with CurryCheck, i.e.,
all modules are compiled and possible properties contained
in the modules are tested with automatically generated test data.
Moreover, one can also specify test suites.
More details about these commands can be found in
Sections~\ref{sec:cmd-check} and~\ref{sec:cmd-test}.
...
use the \code{source}\pindex{source} key. For a HTTP source, use:
%
\begin{lstlisting}
{
  ...,
...
version number. Thus, the tag \ccode{\$version}\pindex{\$version} is equivalent
...
\section{The \texttt{base} Package}
\label{sec:basepackage}

Every Curry distribution comes with some libraries
to support basic operations, like list operations,
actions to read and write files, etc.
The most prominent library is the \code{Prelude}\pindex{Prelude}
which is implicitly added to the import list of a Curry module.
In order to manage different versions of these base libraries,
there is a distinguished package \code{base}\pindex{base} containing
these libraries.
Thus, one can include a dependency to a specific version
of the \code{base} package in the \code{package.json} file, e.g.:
%
\begin{lstlisting}
{
  ...,
...
\end{lstlisting}
%
Each Curry system comes with a specific version of the \code{base}
package. This version can be queried with the option \code{--base-version},
e.g.,
%
\begin{lstlisting}
> pakcs --base-version
...
\end{lstlisting}
%
If CPM tries to install a package, it adds the condition
that the version of package \code{base} must be identical to the \code{base}
version of the compiler used by CPM.
For instance, if \code{pakcs} is used by CPM
(see Section~\ref{sec:config} how to change the default compiler of CPM),
the dependencies shown above are changed to
%
\begin{lstlisting}
{
  ...,
  "dependencies": {
    "base": "= 3.0.0, >= 2.1.0, < 3.0.0",
    "html": ">= 2.0.0, < 2.2.0"
    "json": "~1.1.0"
  }
  ...
...
\end{lstlisting}
%
before resolving all constraints.
Of course, in this case, the constraints are not solvable
so that one has to choose another Curry compiler for this package.
...
\end{lstlisting}
For instance, the package \code{cpm} with version \code{3.1.0}
is downloaded from
\begin{lstlisting}
https://cpm.curry-lang.org/PACKAGES/cpm-3.1.0.tar.gz
\end{lstlisting}
...
This section provides a short description of all available CPM commands.

\subsection{General options}

There are some general global options
which can be placed in front of any CPM command:
\begin{description}
...
\end{description}
%
For instance, to see many details when CPM lists all packages
(e.g., configuration, reading files) together with timings,
one can use the command
\begin{lstlisting}
cypm -v debug -t list
\end{lstlisting}

\subsection{ \code{add}: Add a package}

\begin{description}
\item[\fbox{\code{add --package $dir$ [--force]}}]\pindex{add}
...
\item[\fbox{\code{add --dependency $package$ [--force]}}]\pindex{add}
...
\item[\fbox{\code{add $package$ [--force]}}]\pindex{add}
...
\subsection{ \code{check}: Check a package}
\label{sec:cmd-check}

\begin{description}
\item[\fbox{\code{check}}]\pindex{check}
Checks the current package for consistency of the dependencies
...
If there is some package in the dependency specification
which is not used, a warning is reported.
The option \code{--info} shows some more information,
e.g., the complete list of all Curry source modules.

In addition to the interface checks, the static source code
of all package modules are examined by CurryCheck in order to
report unintended uses of some Curry functions,
e.g., set functions or non-strict unification (\code{=:<=}).
The latter checks can be suppressed by the option \code{--source}.
\end{description}

\subsection{ \code{checkout}: Checkout a package}

\begin{description}
\item[\fbox{\code{checkout $package$ [--output $d$] [--$pre$]}}]\pindex{checkout}
Checks out the newest version (compatible to the current compiler)
of a package into the local directory \code{$package$}
(if the option \code{--output} is not provided) or into the directory $d$.
\code{--$pre$} enables the installation of pre-release versions.
This command is useful to test the package operations
or install a binary of the package.

\item[\fbox{\code{checkout $package$ $version$ [--output $d$]}}]\pindex{checkout}
...
\subsection{ \code{clean}: Clean the current package}
\label{sec:cmd-clean}

\begin{description}
\item[\fbox{\code{clean [--dependencies]}}]\pindex{clean}
Cleans the current package from the
...
For instance, this command can be used to clean an application package
after installing the application.

If the option \code{-d} or \code{--dependencies} is provided,
then \emph{all} dependencies in the package specification are removed.
This might be reasonable in the ``homepackage'' to remove
locally installed packages.
\end{description}


\subsection{ \code{config}:   Show current configuration of CPM}

\begin{description}
\item[\fbox{\code{config}}]\pindex{config}
...
\subsection{ \code{curry}: Load package spec and start Curry with correct dependencies}

\begin{description}
\item[\fbox{\code{curry $args$}}]\pindex{curry}
Starts the Curry system with the 
dependencies of the current package available.
Any arguments are passed verbatim to the Curry system.
\end{description}


\subsection{ \code{deps}: Calculate and show dependencies}

\begin{description}
\item[\fbox{\code{deps}}]\pindex{deps}
Does a dependency resolution run for the current 
package and prints the results. The result is either a list of all package
versions chosen or a description of the conflict encountered during dependency
resolution.
In order to reduce the amount of information shown,
only the first occurrence of a package is shown.
If the option \code{--full} is given, then also repeated package
dependencies are shown.

If the option \code{--path} is set, only the value of \code{CURRYPATH}
required to load modules of this package is shown.

If the option \code{--language-server} is set, the full path containing
all source directories is written into the file
in the file \ccode{.curry/language-server/paths.json}.
This file is used by the Curry Language Server%
\footnote{\url{https://github.com/fwcd/curry-language-server}}
to find all imported modules of the current package
when it is used in the VS Code source-code editor.
When starting the Curry Language Server with VS Code,
the language server invokes CPM with this option in order to
set its import path.

For older version of the Curry Language Server,
one can also use the option \code{--code} to set the full path
of all source directories as property
\code{curry.languageServer.importPaths}
in the file \ccode{.vscode/settings.json}
(which is updated if it already exists).

If the option \code{--graph} is provided, then the dependency
graph is translated into a graph in
GraphViz\footnote{\url{http://www.graphviz.org/}} format.
With the option \code{--viewgraph}, the dependency graph is visualized
with the command defined in the \code{dotviewcommand} field of the
rc file of the Curry system (e.g., \code{~/.pakcsrc} in case of PAKCS).

\end{description}


\subsection{ \code{diff}: Diff the current package against another version}

\begin{description}
\item[\fbox{\code{diff [$version$]}}]\pindex{diff}
...
\subsection{ \code{doc}: Generation documentation for current package (with CurryDoc)}

\begin{description}
\item[\fbox{\code{doc}}]\pindex{doc}
...
The API documentation in HTML format is generated with CurryDoc,\footnote{%
\url{https://cpm.curry-lang.org/pkgs/currydoc.html}}
i.e., CPM invokes the executable \code{curry-doc} to generate
the HTML documentation.
If the executable of CurryDoc is installed with another name
or in a directory which is not part of the standard load path,
one can specify the name of the executable by the option \code{--executable}.

If the package specification contains a list of exported modules
(see Section~\ref{sec:reference}), then these modules are documented.
If the package specification contains the entry \code{executable}
(see Section~\ref{sec:reference}) but no list of exported modules,
no modules are documented.
Otherwise, all modules in the directory \code{src} or other directories
specified in \code{sourceDirs} field of the package specification
...
already documented\footnote{The default location for the documentation
of all packages is \url{https://cpm.curry-lang.org/DOC/},
i.e., the documentation of package \code{p} with version \code{v}
can be found at \url{https://cpm.curry-lang.org/DOC/p-v}.
This default location can be changed with the option \code{--url}.}
...
\subsection{ \code{exec}: Execute a command with the \code{CURRYPATH} set} 

\begin{description}
\item[\fbox{\code{exec $command$}}]\pindex{exec}
Executes an arbitrary command with the 
...
\subsection{ \code{help}: Show list of all commands and quit}

\begin{description}
\item[\fbox{\code{help}}]\pindex{help}
This command shows a list of all available CPM commands.
In order to see more details about the options of a command,
invoke the command with the operation \code{-h} or \code{--help},
as in
\begin{lstlisting}
cypm install --help
\end{lstlisting}
...
\subsection{ \code{info}: Print package information}

\begin{description}
\item[\fbox{\code{info}}]\pindex{info}
Gives information on the current package, e.g. the
package's name, author, synopsis and its dependency specifications.

\item[\fbox{\code{info $package$ [--all]}}]\pindex{info}
...
\item[\fbox{\code{info $package$ $version$ [--all]}}]\pindex{info}
Prints basic information on the given package version.
The option \code{--all} shows more information.
\end{description}

\subsection{ \code{init}: Initialize the current directory as a package}

\begin{description}
\item[\fbox{\code{init}}]\pindex{init}
Initializes a new project package in the current directory
and puts some template files in this directory.
The name of the new package is the same as the directory name.
\end{description}


\subsection{ \code{install}: Install a package with its dependencies}

\begin{description}
\item[\fbox{\code{install}}]\pindex{install}
Installs all dependencies of the current package.
Furthermore, if the current package contains also executables
for an application,
the executables specified in the package are compiled and installed
(unless the option \code{-n} or \code{--noexec} is set).
With the option \code{-x} or \code{--exec}, the executables
are installed without installing all dependencies again.
This is useful to speed up the re-installation of a previously
installed application.
If a name is added to the option \code{-x} or \code{--exec}, e.g.,
%
\begin{lstlisting}
> cypm install --exec compiler
\end{lstlisting}
%
then only the executable with this name is installed.

\item[\fbox{\code{install $package$ [--$pre$]}}]\pindex{install}
...
\item[\fbox{\code{install $package$ $version$}}]\pindex{install}
...
\item[\fbox{\code{install $package$.zip}}]\pindex{install}
...
\subsection{ \code{link}: Link a package to the local cache}

\begin{description}
\item[\fbox{\code{link $source$}}]\pindex{link}
Can be used to replace a dependency of the 
current package using a local copy, see Section~\ref{sec:cpm-link} for details.
\end{description}


\subsection{ \code{list}: List all packages of the repository}

\begin{description}
\item[\fbox{\code{list [--system] [--versions] [--csv]}}]\pindex{list}
...
\item[\fbox{\code{list --category [--csv]}}]\pindex{list}
...
\subsection{ \code{new}: Create a new package}

\begin{description}
\item[\fbox{\code{new $project$}}]\pindex{new}
Creates a new project package with the given name and some template files
in the directory $project$.
\end{description}


\subsection{ \code{repl}: Load package spec and start Curry REPL with correct dependencies}

\begin{description}
\item[\fbox{\code{repl $args$}}]\pindex{repl}
Starts the Curry system with the 
dependencies of the current package available.
Any arguments are passed verbatim to the Curry system.
\end{description}
...
\subsection{ \code{run}: Compile and run a package executable}

\begin{description}
\item[\fbox{\code{run $args$}}]\pindex{run}
Compiles and runs the executable specified in the description
of the current package (see Section~\ref{sec:reference}).
If the package specifies more than one executable,
the first argument following \ccode{run} should be the
name of the executable, otherwise the first executable is selected.
If one wants to pass run-time arguments to the executable,
they should be prefixed by \ccode{--}.
For instance, if the executable has the name \code{currytest},
it can be compiled and executed with the run-time arguments
\ccode{first second} by
%
\begin{lstlisting}
cypm run currytest -- first second
\end{lstlisting}
%
If \code{currytest} is the only executable of the package,
the command can be abbreviated to
%
\begin{lstlisting}
cypm run -- first second
\end{lstlisting}
%
If \code{CurryTest} is the main module of this executable,
then the previous command is a short form of the command
%
\begin{lstlisting}
cypm curry :load CurryTest :eval main :quit -- first second
\end{lstlisting}
\end{description}


\subsection{ \code{search}: Search in the package repository}

\begin{description}
\item[\fbox{\code{search [--module|--exec] $query$}}]\pindex{search}
...
\subsection{ \code{test}: Test the current package (with CurryCheck)}
\label{sec:cmd-test}

\begin{description}
\item[\fbox{\code{test [--modules $\mathit{modules}$] [--compile]}}]\pindex{test}
...
If the executable \code{curry-check} is not installed or the
option \code{--compile} is provided, then the modules are only compiled
and not tested.
Hence, a package can be checked for compilation errors
by the command
\begin{lstlisting}
cypm test --compile
\end{lstlisting}
...
\subsection{ \code{uninstall}: Uninstall a package}

\begin{description}
\item[\fbox{\code{uninstall}}]\pindex{uninstall}
Uninstalls the executable installed for the current package.

\item[\fbox{\code{uninstall $package$}}]\pindex{uninstall}
Uninstalls the executable and the cached copy of a package
which has been installed by the \code{install} command.

\item[\fbox{\code{uninstall $package$ $version$}}]\pindex{uninstall}
Uninstalls a specific version
of a package from the global package cache.
\end{description}


\subsection{ \code{upgrade}: Upgrade one or more packages}

\begin{description}
\item[\fbox{\code{upgrade}}]\pindex{upgrade}
Upgrades all dependencies of the current package to
the newest compatible version.

\item[\fbox{\code{upgrade $package$}}]\pindex{upgrade}
Upgrades a specific dependency of the
current package and all its transitive dependencies to their newest compatible
versions.
\end{description}


\subsection{ \code{update}: Update the package index}

\begin{description}
\item[\fbox{\code{update}}]\pindex{update}
...
\subsection{ \code{upload}: Upload current package to Masala/CPM package server}

\begin{description}
\item[\fbox{\code{upload [--tag] [--nopublish] [--force] [--login $name$] [--password $passwd$]}}]\pindex{upload}
Uploads the current package to the central package index managed by
Masala.\index{Masala}%
\footnote{See \url{https://cpm.curry-lang.org/masala/}}
Thus, one has to be registered to Masala (see Masala's homepage
for further information).
Registration is free but only trusted users or the Masala administrator
can publish the package so that it can be used by other developers via CPM
(after updating their local copy of the central package index
by \ccode{cypm update}).
Uploading requires that the package has a public source location, e.g.,
%
\begin{lstlisting}
{
  ...,
  "source": {
    "git": "https://github.com/curry-packages/$\ldots$.git",
    "tag": "$\$$version"
  }
}
\end{lstlisting}
%
Thus, the uploaded package will be taken from this public source
and \textbf{not from the local files}.

If the source location is a git repository with tag
\ccode{$\$$version} (as shown above) and the upload option \code{--tag} is set,
...
If the option \code{--tag} is not provided, the user will be asked
whether the repository should be tagged (if it is a git repository).

Before uploading the package to Masala,
the remote repository will be cloned and tested (by \ccode{cypm test}).
...
of the central package index) and then published
(unless the option \ccode{--nopublish} is set or the user is not
a trusted Masala user).

One can also use the options \code{--login} and \code{--password}
to provide the login name and password of Masala at the command line.
Otherwise, one will be asked for this information after the
package has been successfully tested.

Users registered with administrator rights at Masala can also
use the option \ccode{--force} to overwrite
...
\begin{description}
\item[\fbox{\code{name*}}]\pindex{name}
The name of the package. Must only contain ASCII 
letters, digits, hyphens and underscores. Must start with a letter.

\item[\fbox{\code{version*}}]\pindex{version}
The version of the package. Must follow the format
for semantic versioning version numbers.

\item[\fbox{\code{author*}}]\pindex{author}
The package's author. This is a free-form field, 
...
\item[\fbox{\code{maintainer}}]\pindex{maintainer}
The current maintainers of the package, if 
...
\item[\fbox{\code{synopsis*}}]\pindex{synopsis}
A short form summary of the package's purpose.
It should be kept as short as possible (ideally, less than 100 characters).

\item[\fbox{\code{description}}]\pindex{description}
A longer form description of what the package does.

\item[\fbox{\code{category}}]\pindex{category}
A list of keywords that characterize the main area where the
package can be used, e.g., \code{Data}, \code{Numeric}, \code{GUI},
\code{Web}, etc.

\item[\fbox{\code{license}}]\pindex{license}
...
\item[\fbox{\code{licenseFile}}]\pindex{licenseFile}
The name of a file in the root directory of the
...
\item[\fbox{\code{copyright}}]\pindex{copyright}
Copyright information regarding the package.

\item[\fbox{\code{homepage}}]\pindex{homepage}
The package's web site. This field should contain a valid URL.

\item[\fbox{\code{bugReports}}]\pindex{bugReports}
A place to report bugs found in the package.
The suggested formats are either a valid URL to a bug tracker
or an email address.

\item[\fbox{\code{repository}}]\pindex{repository}
...
\item[\fbox{\code{dependencies*}}]\pindex{dependencies}
...
\item[\fbox{\code{compilerCompatibility}}]\pindex{compilerCompatibility}
The package's compatibility to 
...
\item[\fbox{\code{source}}]\pindex{source}
...
\item[\fbox{\code{sourceDirs}}]\pindex{sourceDirs}
A list of directories inside this
...
\item[\fbox{\code{exportedModules}}]\pindex{exportedModules}
...
\item[\fbox{\code{configModule}}]\pindex{configModule}
...
\item[\fbox{\code{executable}}]\pindex{executable}
...
\item[\fbox{\code{executables}}]\pindex{executables}
It is also possible to specify more than one executable
in package which will be installed by the command \code{cypm install}.
In this case, one can use \code{executables}
instead of \code{executable}.
The value of \code{executables} is an array of JSON objects as used
as \code{executable} values.
For instance, a possible specification could be
%
\begin{lstlisting}
{
  ...,
...
\item[\fbox{\code{testsuite}}]\pindex{testsuite}
...
\item[\fbox{\code{documentation}}]\pindex{documentation}
...
generated auxiliary files (see Section~\ref{sec:cmd-clean}).
